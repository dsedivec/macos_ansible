...
def install_ports(module, port_path, ports, variant):
    ...
    for port in ports:
        just_port_name = port.split()[0]
        if query_port(module, port_path, just_port_name):
            continue

        cmd = '%s install %s' % (port_path, port)
        if variant:
            cmd += ' %s' % (variant,)
        rc, out, err = module.run_command(cmd)

        if not query_port(module, port_path, just_port_name):
